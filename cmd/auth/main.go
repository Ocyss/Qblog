...
import (
	...
	"github.com/Ocyss/Qblog/cmd/auth/biz/dal"
	"github.com/Ocyss/Qblog/cmd/auth/biz/rpc"
...
func Init() {
	tracer2.InitJaeger(constants.AuthServiceName)
	dal.Init()
	rpc.Init()
}
...
func main() {
	...
	svr := authservice.NewServer(new(AuthServiceImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constants.AuthServiceName}), // server name
	...
	if err := svr.Run(); err != nil {
		klog.Fatalf("%s server stopped with error: %v", constants.AuthServiceName, err)
	}
}
