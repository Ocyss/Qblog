...
func NewStatisticsClient() *StatisticsClient {
	r, err := etcd.NewEtcdResolver(configs.GetRegistry().Address)
	if err != nil {
		klog.Fatalf("new resolver failed: %v", err.Error())
	}

	c, err := statisticsservice.NewClient(constants.StatisticsServiceName,
	...
	if err != nil {
		klog.Fatalf("new %s client failed: %v", constants.StatisticsServiceName, err.Error())
	}

	return &StatisticsClient{Client: c}
}
